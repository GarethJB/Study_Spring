...
select * from ( select department_name, to_char(hire_date,'yyyy') unit from employees e inner 
join (select rank, department_id, '(TOP'||rank||')' || department_name department_name from 
(select department_id, dense_rank() over(order by count(*) desc) rank from employees where 
to_char(hire_date,'yyyy') between '2005' and '2008' group by department_id) e left outer join 
departments d using(department_id) where rank <= 3) r using(department_id) ) pivot( count(unit) 
for UNIT in ( '2005' y2005, '2006' y2006, '2007' y2007, '2008' y2008 ) ) order by 1 
